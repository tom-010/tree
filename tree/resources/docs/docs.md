...
The `home-routes` handler in the `tree.routes.home` namespace
...
The `home-page` function will in turn call the `tree.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/tree/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `tree.handler` namespace:
...
Request middleware functions are located under the `tree.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `tree.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `tree.db.core` in some other namespace.
